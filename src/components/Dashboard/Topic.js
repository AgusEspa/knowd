...
const Topic = (props) => {
	...
	const handleDeleteTopic = async (event, topicId) => {
		...
		try {
			...
			const editedSubject = {
				...
				topics: fetchedEditedSubject[0].topics.filter(
	...
	const editTopic = async (topicId, title, checked) => {
		...
		try {
			...
			const editedSubject = {
				...fetchedEditedSubject[0],
				topics: fetchedEditedSubject[0].topics
	...
	return (
		<>
			<input
				...
				checked={isDone}
				onChange={(event) =>
					handleCheckbox(event, props.topic.id, props.topic.title)
				}
			/>
			<label>
				{props.topic.title === "" ? "*empty*" : props.topic.title}
			</label>
			...
			{isLoadingDelete ? (
				...
				<div className={styles.deleteButtonContainer}>
					<button
						onClick={(event) =>
							handleDeleteTopic(event, props.topic.id)
						}
