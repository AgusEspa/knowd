...
const Stats = (props) => {
	...
	const buildFieldStats = () => {
		...
		props.fields.forEach((fieldObject) => {
			statsByAllFields.push({
				...
				valueL: props.subjects.filter(
					(subject) =>
						subject.field === fieldObject.field &&
						subject.status === "Learning"
				).length,
				valueM: props.subjects.filter(
					(subject) =>
						subject.field === fieldObject.field &&
						subject.status === "Mastered"
				).length,
				valueW: props.subjects.filter(
					(subject) =>
						subject.field === fieldObject.field &&
						subject.status === "Wish"
				).length,
		...
		if (
			props.subjects.filter((subject) => subject.field === "").length > 0
		) {
			statsByAllFields.push({
				name: "Empty Field",
				valueL: props.subjects.filter(
					(subject) =>
						subject.field === "" && subject.status === "Learning"
				).length,
				valueM: props.subjects.filter(
					(subject) =>
						subject.field === "" && subject.status === "Mastered"
				).length,
				valueW: props.subjects.filter(
					(subject) =>
						subject.field === "" && subject.status === "Wish"
				).length,
			});
		}
	...
	const relCoefficient =
		relationsData.length > 0
			? (
					relationsData[0].Total_rels / relationsData[0].Total_subs
			  ).toFixed(2)
			: 0;
	...
	const buildAreasStats = () => {
		...
		props.fields
			.filter((fieldObject) => fieldObject.areas.length >= 2)
			.forEach((fieldObject) => {
				const data = fieldObject.areas.map((area) => ({
					name: area.area,
					value: props.subjects.filter(
						(subject) =>
							subject.field === fieldObject.field &&
							subject.area === area.area &&
							subject.status !== "Wish"
					).length,
				}));

				if (
					props.subjects.filter((subject) => subject.area === "")
						.length > 0
				) {
					data.push({
						name: "Empty Area",
		...
		<main className={styles.statsContainer}>
			{props.fields.length < 3 && (
				<div className={styles.statsBigBox}>
					<p>
						You don't have enough subjects and fields to calculate
						stats. To see all the stats, add more subjects and
						relations.
					</p>
				</div>
			)}
