...
const Subjects = (props) => {
			...
			<ul>
				<li>
					<button
						...
						className={
							activeField.field === fieldObject.field &&
							activeField.area === ""
								? styles.activeButton
	...
	const displaySubjects = () => {
		if (activeField.field === "") {
			...
			return props.subjects.filter(
				(subject) => subject.field === activeField.field
			);
		} else {
			return props.subjects.filter(
				(subject) =>
					subject.field === activeField.field &&
					subject.area === activeField.area
			);
		}
	};
	...
	const mappedSearchedLearningSubjects = searchSubjects()
		.filter((subject) => subject.status === "Learning")
		.sort(sortFunction)
		.map((subject) => (
			<Subject
				key={subject.id}
	...
	const mappedSearchedWishSubjects = searchSubjects()
		.filter((subject) => subject.status === "Wish")
		.sort(sortFunction)
		.map((subject) => (
			<Subject
				key={subject.id}
	...
	const mappedSearchedMasteredSubjects = searchSubjects()
		.filter((subject) => subject.status === "Mastered")
		.sort(sortFunction)
		.map((subject) => (
			<Subject
				key={subject.id}
	...
	return (
		<>
			...
			{props.newSubjectsWindowIsOpen && (
				<SubjectCreate
					setNewSubjectsWindowIsOpen={
						props.setNewSubjectsWindowIsOpen
					}
					setSubjects={props.setSubjects}
