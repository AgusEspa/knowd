...
const Relations = (props) => {
	...
	const handleNewRelation = async (event) => {
		...
		try {
			const response = await api.post(
				`/subjects/${props.subjectId}/relations`,
				relationFormData
			);
			...
			const editedSubject = {
				...
				relations: fetchedEditedSubject[0].relations.concat(
					response.data
				),
			};
	...
	const mappedRelations = props.relations
		.sort(sortFunction)
		.map((relation) => (
			<li key={relation.id}>
				<Relation
	...
	const subjectOptions = props.subjects
		.filter((subject) => subject.field !== props.field)
		.map((subject) => <option key={subject.id}>{subject.title}</option>);
		...
		<>
			{isLoadingNew || isLoadingTopic ? (
				...
				<div
					...
					onClick={() => props.setRelationsWindowIsOpen(false)}
				/>
			)}
