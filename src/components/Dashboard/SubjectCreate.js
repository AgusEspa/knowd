...
const SubjectCreate = (props) => {
	...
	const handleCreateSubject = async (event) => {
		...
		try {
			...
			if (!error.response || error.response.status >= 500) {
				props.setNetworkNotification((prevState) => ({
					message:
	...
	const selectedField = props.fields.find(
		(fieldObject) => fieldObject.field === subjectFormData.field
	);
		...
		<>
			{isLoading ? (
				...
				<div
					...
					onClick={() => props.setNewSubjectsWindowIsOpen(false)}
				/>
			)}
				...
				<div className={styles.editWindow}>
					<form onSubmit={handleCreateSubject}>
						...
						<div className={styles.inputBox}>
							...
							{subjectFormData.dueDate === "" ||
							subjectFormData.dueDate === null ? (
								<button
									type="button"
